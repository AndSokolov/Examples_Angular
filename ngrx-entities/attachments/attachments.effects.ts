...
import { LoadFail, LoadFiles, LoadFilesSuccess } from './attachments.actions';
...
export class AttachmentsEffects {
	...
	public loadFiles$ = this.actions$.pipe(
		...
		mergeMap((action: LoadFiles) => this.service.getFileList(action.systemId, action.infoType).pipe(
